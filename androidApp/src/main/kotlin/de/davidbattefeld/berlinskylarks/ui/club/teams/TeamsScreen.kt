...
fun TeamsScreen(
    ...
    Scaffold(
        ...
        bottomBar = { SkylarksBottomBar(topLevelBackStack, navigationType) }) { paddingValues ->

        LazyColumn(
            state = listState, modifier = Modifier.padding(paddingValues)
        ) {
            when (vm.viewState) {
                ...
                ViewState.Found -> {
                    itemsIndexed(teams) { index, team ->
                        Column {
                            TeamsRow(
                                team = team, modifier = Modifier.clickable {
